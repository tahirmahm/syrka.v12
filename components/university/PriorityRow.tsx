...
import type { InstitutionalPriority } from '@/lib/campus-types'

const URGENCY_TONE = { high: 'red', medium: 'amber', low: 'neutral' } as const

export function PriorityRow({ priority }: { priority: InstitutionalPriority }) {
  return (
    <li className="flex flex-col gap-1.5 border-b border-campus-border py-4 last:border-b-0">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <p className="font-campus-sans text-campus-sm font-medium text-campus-text">{priority.title}</p>
      </div>
      <p className="font-campus-sans text-campus-sm text-campus-muted">{priority.rationale}</p>
      <p className="font-campus-mono text-campus-xs text-campus-muted">
        Recommended: {priority.recommendedIntervention} · Owner: {priority.owner}
      </p>
